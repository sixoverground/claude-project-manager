...
5. **AI reviewer satisfied.** Reviewer-agnostic and anchored to the head commit SHA (no timestamps, no marker commits). Gate 5 passes when BOTH hold, via a single GraphQL query for `headRefOid`, `reviews { author.login, commit.oid }`, `reviewThreads { isResolved, comments { author.login, commit.oid } }`, and the head commit's `checkSuites { checkRuns { name, conclusion } }`:
   - **Reviewed the head.** Proven by any of: a review whose `commit.oid == headRefOid`; an inline review-thread comment whose `commit.oid == headRefOid`; or a successful check run whose name matches `yolo_reviewer.check` on the head commit. The check-run anchor is how a CI-based reviewer (the Claude review Action) proves it evaluated the head even on a clean pass that leaves no comment, which is the normal terminal state after the routine fixes findings and the head moves. Login matching is case-insensitive and strips a trailing `[bot]` (GraphQL reports bot actors without that suffix); check-name matching is case-insensitive.
   - **All threads resolved.** Every `reviewThread` on the PR has `isResolved == true`.

   Configured per project via `yolo_reviewer` (repo-overridable): `{ "logins": [...], "check": "<check-run name>" }`. Omitted defaults to Copilot (`copilot-pull-request-reviewer`, `github-copilot`, `copilot`) with no check. `"yolo_reviewer": false` skips gate 5 entirely. Because only diff-anchored review comments form resolvable threads, the reviewer must post **inline** review comments (Copilot does natively; Claude's review workflow uses `--comment`), and re-review-on-push keeps the anchors tracking the head (Copilot: "Review new pushes" ruleset; Claude: `synchronize` trigger). The dispatched routine addresses each thread and resolves it via the `resolveReviewThread` GraphQL mutation; a human can un-resolve to block.
